...
export class LocalController {
  constructor(node, bus) {
    ...
    this.pedRequestPending = false;  // mid-block ped asked for early gap-out
  }
  ...
  tick(t, dt, sensors) {
    ...
    for (const m of this.bus.drain(this.node)) {
      switch (m.type) {
        ...
        case MSG.PED_REQUEST:
          // Mid-block pedestrian controller requests early gap-out after gMin.
          // We set a flag; actuatedStep will respect it identically to a gap-out
          // (treat current phase as having no demand once gMin is served).
          this.pedRequestPending = true;
          break;
      }
    }
    ...
    if (!preempted) {
      ...
      else actuatedStep(this.machine, sensors, boost, this.pedRequestPending);
    }
    // Clear ped request once the protecting phase ends (we've created the gap)
    if (this.pedRequestPending && this.machine.state !== 'GREEN') {
      this.pedRequestPending = false;
    }
