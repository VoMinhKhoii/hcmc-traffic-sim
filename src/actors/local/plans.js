 ...
 * `pedRequest` — when true, treat the current phase as having no vehicle detection
 * once gMin is served (mid-block pedestrian asked for early gap-out).
 */
export function actuatedStep(machine, sensors, boost = 1, pedRequest = false) {
  ...
  // pedRequest forces a gap-out once gMin is served, even if vehicles are still arriving
  const gapped = pedRequest
    ? tG >= minG
    : !PHASES[cur].some((d) => sensors[d].detection);
