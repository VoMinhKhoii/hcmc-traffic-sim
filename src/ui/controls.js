...
import { CROSSINGS, MID_BLOCK_PEDS } from '../network.js';
...
export function buildControls(root, sim) {
  root.innerHTML = `
    ...
    <div class="grp"><label>Mid-block pedestrian <kbd>R</kbd></label>
      ${Object.keys(MID_BLOCK_PEDS).map((id) =>
        `<button id="b-mbp-${id}">🚶 Ped button (I2–I4 mid)</button>`).join('')}</div>
  ...
  for (const id of Object.keys(MID_BLOCK_PEDS)) {
    $(`#b-mbp-${id}`).onclick = () => sim.pressMidBlockPed(id);
  }

  // Press R to trigger all mid-block pedestrian buttons
  document.addEventListener('keydown', (e) => {
    if (e.key === 'r' || e.key === 'R') {
      for (const id of Object.keys(MID_BLOCK_PEDS)) {
        sim.pressMidBlockPed(id);
      }
    }
  });
