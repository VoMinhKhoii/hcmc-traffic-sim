...
import {
  ...
  MAIN_PHASE, MID_BLOCK_PEDS, linkById, approachVector, feedsApproach,
} from '../network.js';
...
export function draw(ctx, sim) {
  ...
  for (const p of Object.values(sim.model.pipes)) {
    ...
    for (const e of p.entries) {
      ...
      if (e.pedHeld) f = Math.min(f, p.pedFrac - 0.015);
  ...
  for (const [id, spec] of Object.entries(MID_BLOCK_PEDS)) {
    const mbp = sim.midBlockPeds[id];
    if (!mbp) continue;
    const l = linkById(spec.linkId);
    const A = INTERSECTIONS[l.a], B = INTERSECTIONS[l.b];
    const px = A.x + (B.x - A.x) * spec.frac;
    const py = A.y + (B.y - A.y) * spec.frac;
    drawMidBlockPed(ctx, px, py, A, B, mbp.view(), t);
  }
...
// ---- mid-block pedestrian signal head -------------------------------------
// Pedestrian head: right side of A→B traffic. Two aspects: red/green.
// Vehicle heads: one per direction, 3-aspect. During off-peak countdown the
// vehicle aspect cycles green→yellow→red in sync with the MBP state machine.
function drawMidBlockPed(ctx, px, py, A, B, view, t) {
  const dist = Math.hypot(B.x - A.x, B.y - A.y);
  const ux = (B.x - A.x) / dist, uy = (B.y - A.y) / dist;
  const nx = -uy, ny = ux;

  const walkLit   = view.walk;
  const waitBlink = view.waiting && Math.floor(t * 2) % 2 === 0;
  const vAspect   = view.vehicleAspect ?? (walkLit ? 'red' : 'green');

  // ---- pedestrian head: right of A→B (left-normal side) ------------------
  const pedOff = HALF + 20;
  const hx = px + nx * pedOff, hy = py + ny * pedOff;
  const hw = 13, hh = 30, pr = 5;
  ctx.fillStyle = COL.head;
  roundRect(ctx, hx - hw / 2, hy - hh / 2, hw, hh, 4);
  ctx.fill();
  ctx.fillStyle = (!walkLit) ? (waitBlink ? '#ff6f6f' : COL.red) : COL.unlit;
  ctx.beginPath(); ctx.arc(hx, hy - pr, 4, 0, 7); ctx.fill();
  ctx.fillStyle = walkLit ? COL.green : COL.unlit;
  ctx.beginPath(); ctx.arc(hx, hy + pr, 4, 0, 7); ctx.fill();
  if (walkLit) {
    statusBadge(ctx, hx, hy - hh / 2 - 10, 'WALK', COL.walk);
  } else if (view.waiting) {
    statusBadge(ctx, hx, hy - hh / 2 - 10, 'WAIT', COL.yellow);
  }
  label(ctx, hx + hw / 2 + 4, hy, 'MBP', COL.label, 9, false, 'left', 'middle');

  // ---- vehicle signal heads: one per traffic direction --------------------
  const vhw = 11, vhh = 28;
  const vOff = HALF + 32;

  // Head for A→B traffic: negative-normal side
  const v1x = px - nx * vOff, v1y = py - ny * vOff;
  ctx.fillStyle = COL.head;
  roundRect(ctx, v1x - vhw / 2, v1y - vhh / 2, vhw, vhh, 3);
  ctx.fill();
  ['red', 'yellow', 'green'].forEach((c, i) => {
    ctx.fillStyle = c === vAspect ? COL[c] : COL.unlit;
    ctx.beginPath(); ctx.arc(v1x, v1y - 8 + i * 8, 3.2, 0, 7);
    ctx.fill();
  });

  // Head for B→A traffic: positive-normal side
  const v2x = px + nx * vOff, v2y = py + ny * vOff;
  ctx.fillStyle = COL.head;
  roundRect(ctx, v2x - vhw / 2, v2y - vhh / 2, vhw, vhh, 3);
  ctx.fill();
  ['red', 'yellow', 'green'].forEach((c, i) => {
    ctx.fillStyle = c === vAspect ? COL[c] : COL.unlit;
    ctx.beginPath(); ctx.arc(v2x, v2y - 8 + i * 8, 3.2, 0, 7);
    ctx.fill();
  });

  // countdown badge between the two vehicle heads
  if (view.countdown !== null && view.countdown > 0) {
    const badgeCol = vAspect === 'green' ? COL.green
                   : vAspect === 'yellow' ? COL.yellow : COL.red;
    statusBadge(ctx, px, py - HALF - 14, `${view.countdown}s`, badgeCol);
  }
}
...
function drawLegend(ctx, x, y) {
  const rows = [
    ...
    ['mbp', COL.walk, 'MBP = mid-block pedestrian light (I2–I4); WAIT = button pressed'],
  ...
  rows.forEach((r, i) => {
    ...
    if (r[0] === 'dot') { ctx.fillStyle = r[1]; ctx.beginPath(); ctx.arc(x + 7, yy - 4, 4.5, 0, 7); ctx.fill(); }
      ...
      ctx.fillStyle = COL.head; roundRect(ctx, x, yy - 9, 13, 14, 3); ctx.fill();
      ctx.fillStyle = COL.red; ctx.beginPath(); ctx.arc(x + 6.5, yy - 4, 3, 0, 7); ctx.fill();
      ctx.fillStyle = r[1];    ctx.beginPath(); ctx.arc(x + 6.5, yy + 2, 3, 0, 7); ctx.fill();
    }
    else if (r[0] === 'head') {
