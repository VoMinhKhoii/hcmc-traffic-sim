...
// Drawn perpendicular to the road, on the right-hand side of traffic flowing
// from A→B. Two aspects only: red (DON'T WALK) and green (WALK).
function drawMidBlockPed(ctx, px, py, A, B, view, t) {
  // Unit vector along road A→B, and the right-hand normal (perpendicular)
  const d = Math.hypot(B.x - A.x, B.y - A.y);
  const ux = (B.x - A.x) / d, uy = (B.y - A.y) / d;  // along road
  const nx = -uy, ny = ux;                              // left normal → negate for right

  // Place the head to the right of A→B traffic (i.e. left normal side)
  const offset = HALF + 20;
  const hx = px + nx * offset;
  const hy = py + ny * offset;

  // 2-aspect vertical head (red on top, green on bottom)
  const hw = 13, hh = 30, r = 5;
  ctx.fillStyle = COL.head;
  ...
  ctx.beginPath(); ctx.arc(hx, hy - r, 4, 0, 7); ctx.fill();

  // Green aspect (bottom)
  ctx.fillStyle = walkLit ? COL.green : COL.unlit;
  ctx.beginPath(); ctx.arc(hx, hy + r, 4, 0, 7); ctx.fill();

  // WALK badge above the head when active
  if (walkLit) {
