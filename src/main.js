...
export class Simulation {
  ...
  step() {
    ...
    // mid-block pedestrian controllers — must run before model.step() so
    // blockingTraffic is up-to-date when queues compute discharge.
    ...
    // build map of links currently blocked by a pedestrian WALK phase.
    // frac is the stop position measured from the canonical link.a end.
    const mbpBlocked = {};
    for (const [id, mbp] of Object.entries(this.midBlockPeds)) {
      if (mbp.blockingTraffic) {
        mbpBlocked[mbp.spec.linkId] = { frac: mbp.spec.frac };
      }
    }
    ...
    this.model.step(dt, t, mode, lights, gatesDown, trainAt, mbpBlocked);
